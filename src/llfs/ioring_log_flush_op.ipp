...
inline void BasicIoRingLogFlushOp<DriverImpl>::initialize(DriverImpl* driver)
{
  ...
  this->driver_ = driver;
  this->page_block_.reset(new PackedLogPageBuffer[this->driver_->pages_per_block()]);

  BATT_CHECK_EQ(this->driver_->pages_per_block() * sizeof(PackedLogPageBuffer),
  ...
  //
  const slot_offset_type next_commit_slot_offset = driver->get_commit_pos() + 1;
  ...
  const usize ahead_of_next = [&] {
    if (my_index < next_commit_op_index) {
      return my_index + driver->queue_depth() - next_commit_op_index;
    } else {
  ...
  // Initialize the log page header.
  {
    ...
    header->slot_offset =
        next_commit_block_slot_range.lower_bound + driver->block_capacity() * ahead_of_next;
    ...
    this->file_offset_ = driver->file_offset_from_slot(header->slot_offset);
...
inline void BasicIoRingLogFlushOp<DriverImpl>::activate()
{
  ...
  header->reset(/*slot_offset=*/my_index * this->driver_->block_capacity());
...
inline void BasicIoRingLogFlushOp<DriverImpl>::handle_flush(const StatusOr<i32>& result)
{
  ...
  if (header->commit_size == this->driver_->block_capacity()) {
    ...
    header->commit_size = 0;
    this->file_offset_ = (this->file_offset_ - this->driver_->block_0_file_offset() +
                          this->driver_->block_size() * this->driver_->queue_depth()) %
                             this->driver_->physical_size() +
                         this->driver_->block_0_file_offset();
  }
